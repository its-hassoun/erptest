...
import type { User } from '../../../types/user';import {
...
export function SubClientDashboard({ tickets, currentUser }: SubClientDashboardProps) {
  ...
  const stats = [
    ...
    {
      label: 'In Progress',
      value: tickets.filter((t) => t.statut === StatutTicket.Ouvert || t.statut === StatutTicket.EnPause
).length,
      icon: Clock,
      color: 'bg-amber-500'
    },
    {
      label: 'Resolved',
      value: tickets.filter((t) => t.statut === StatutTicket.Clos).length,
      icon: CheckCircle,
      color: 'bg-green-500'
    }
  ];
