...
export function AllAgents({ agents, tickets }: AllAgentsProps) {
  ...
  const getAgentStats = (agentId: number) => {
    const assigned = tickets.filter((t) => t.agentPrincipalId === agentId && t.statut !== StatutTicket.Clos).length;
const resolved = tickets.filter((t) => t.agentPrincipalId === agentId && t.statut === StatutTicket.Clos).length; 
